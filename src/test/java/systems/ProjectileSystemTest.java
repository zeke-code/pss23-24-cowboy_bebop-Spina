...
public class ProjectileSystemTest {
  ...
  void projectileTimeAliveIncreasesWithDeltaTime() {
    ...
    assertEquals(0.0, projectileComponent.getTimeAlive(), "Initial time alive should be zero");
    ...
    assertEquals(
        deltaTime, projectileComponent.getTimeAlive(), "Time alive should increase by deltaTime");
    ...
    assertEquals(
        deltaTime * 2,
        projectileComponent.getTimeAlive(),
  ...
  void projectilesRemovedAfterLifetimeExpires() {
    ...
    assertTrue(
        world.getEntities().contains(projectile), "Projectile should be in the world initially");
    ...
    assertTrue(
        world.getEntities().contains(projectile),
  ...
  void multipleProjectilesTrackedIndependently() {
    ...
    assertTrue(
        world.getEntities().contains(projectile1),
        "Long-lifetime projectile should remain in the world");

    // Second projectile should be removed
    assertFalse(
        world.getEntities().contains(projectile2), "Short-lifetime projectile should be removed");

    // First projectile's time should be updated
    assertEquals(
        2.0,
        projectileComponent1.getTimeAlive(),
  ...
  void handleEntitiesWithoutProjectileComponent() {
    ...
    assertTrue(
        world.getEntities().contains(entity), "Non-projectile entity should remain in the world");
  }
  ...
  void handleEmptyWorld() {
    ...
    // Update should not throw exceptions
    assertDoesNotThrow(
        () -> projectileSystem.update(1.0), "Update should not throw exception for empty world");
  }
