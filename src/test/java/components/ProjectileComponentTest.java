...
public class ProjectileComponentTest {
    ...
    void constructorInitializesCorrectly() {
        assertEquals(TEST_DAMAGE, projectile.getDamage(),
                "Damage should match constructor parameter");
        assertEquals(TEST_LIFETIME, projectile.getLifetime(),
                "Lifetime should match constructor parameter");
        assertEquals(TEST_OWNER, projectile.getOwner(),
                "Owner should match constructor parameter");
        assertEquals(0.0, projectile.getTimeAlive(),
                "TimeAlive should be initialized to zero");
    }
    ...
    void damageGetterAndSetter() {
        ...
        assertEquals(newDamage, projectile.getDamage(),
                "Damage should update when set");
    }
    ...
    void lifetimeGetterAndSetter() {
        ...
        assertEquals(newLifetime, projectile.getLifetime(),
                "Lifetime should update when set");
    }
    ...
    void ownerGetterAndSetter() {
        ...
        assertEquals(newOwner, projectile.getOwner(),
                "Owner should update when set");
    }
    ...
    void addTimeAliveIncrementsCorrectly() {
        ...
        assertEquals(increment1, projectile.getTimeAlive(),
                "TimeAlive should increase by the amount added");
        
        double increment2 = 0.5;
        projectile.addTimeAlive(increment2);
        assertEquals(increment1 + increment2, projectile.getTimeAlive(),
    ...
    void shouldDespawnReturnsFalseWhenTimeAliveIsLessThanLifetime() {
        ...
        assertFalse(projectile.shouldDespawn(),
                "Projectile should not despawn when timeAlive < lifetime");
    }
    ...
    void shouldDespawnReturnsTrueWhenTimeAliveEqualsLifetime() {
        ...
        assertTrue(projectile.shouldDespawn(),
                "Projectile should despawn when timeAlive = lifetime");
    }
    ...
    void handlesZeroLifetime() {
        ProjectileComponent zeroLifetimeProjectile = new ProjectileComponent(
                TEST_DAMAGE, 0.0, TEST_OWNER);
        
        // Should despawn immediately with zero lifetime
        assertTrue(zeroLifetimeProjectile.shouldDespawn(),
    ...
    void handlesNegativeTimeAdditions() {
        ...
        assertEquals(initialTimeAlive - 0.5, projectile.getTimeAlive(),
    ...
    void handlesNullOwner() {
        ProjectileComponent nullOwnerProjectile = new ProjectileComponent(
                TEST_DAMAGE, TEST_LIFETIME, null);
        
        assertNull(nullOwnerProjectile.getOwner(),
                "Constructor should accept null owner");
    }
}