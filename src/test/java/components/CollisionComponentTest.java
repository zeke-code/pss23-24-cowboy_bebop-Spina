...
public class CollisionComponentTest {
    ...
    void twoParameterConstructorInitializesCorrectly() {
        ...
        assertEquals(TEST_RADIUS, collision.getRadius(), 
                "Radius should match constructor parameter");
        assertEquals(TEST_IS_SOLID, collision.isSolid(), 
                "isSolid flag should match constructor parameter");
        assertEquals("default", collision.getCollisionGroup(), 
                "Collision group should be set to default");
    }
    
    @Test
    @DisplayName("Three-parameter constructor should initialize all fields")
    void threeParameterConstructorInitializesCorrectly() {
        CollisionComponent collision = new CollisionComponent(TEST_RADIUS, TEST_IS_SOLID, TEST_COLLISION_GROUP);
        
        assertEquals(TEST_RADIUS, collision.getRadius(), 
                "Radius should match constructor parameter");
        assertEquals(TEST_IS_SOLID, collision.isSolid(), 
                "isSolid flag should match constructor parameter");
        assertEquals(TEST_COLLISION_GROUP, collision.getCollisionGroup(), 
    ...
    void radiusGetterAndSetter() {
        ...
        // Test initial value
        assertEquals(TEST_RADIUS, collision.getRadius(), 
                "Initial radius should match constructor value");
        
        // Test setting new value
        double newRadius = 20.0;
        collision.setRadius(newRadius);
        assertEquals(newRadius, collision.getRadius(), 
                "Radius should update when set");
    }
    ...
    void isSolidGetterAndSetter() {
        ...
        // Test initial value
        assertEquals(TEST_IS_SOLID, collision.isSolid(), 
                "Initial isSolid should match constructor value");
        
        // Test toggling the value
        collision.setSolid(!TEST_IS_SOLID);
        assertEquals(!TEST_IS_SOLID, collision.isSolid(), 
                "isSolid should update when set");
    }
    ...
    void collisionGroupGetterAndSetter() {
        ...
        assertEquals(newGroup, collision.getCollisionGroup(), 
                "Collision group should update when set");
    }
    ...
    void handlesZeroRadius() {
        ...
        assertEquals(0.0, collision.getRadius(), 
                "Should allow zero radius for point collisions");
    }
    ...
    void handlesNegativeRadius() {
        ...
        assertEquals(negativeRadius, collision.getRadius(), 
                "Negative radius should be stored as-is");
    }
    ...
    void handlesNullCollisionGroup() {
        ...
        assertNull(collision.getCollisionGroup(), 
                "Should allow setting collision group to null");
    }
    ...
    void handlesEmptyCollisionGroup() {
        CollisionComponent collision = new CollisionComponent(TEST_RADIUS, TEST_IS_SOLID, "");
        assertEquals("", collision.getCollisionGroup(), 
                "Should allow empty string as collision group");
    }
}